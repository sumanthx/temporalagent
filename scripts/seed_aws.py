...
from temporal_agent.aws_backend import DynamoTemporalGraphStore, archive_fixtures
...
def main():
    ...
    keys = archive_fixtures(args.bucket)
    local = build_demo()
    remote = DynamoTemporalGraphStore(args.table, args.region)
    remote.records = local.store.records
    count = remote.persist_all()
    ...
    print(json.dumps({"archived_objects": len(keys), "temporal_facts": count,
